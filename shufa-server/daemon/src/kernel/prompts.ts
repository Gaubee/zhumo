...
export function buildTaskPrompt(input: TaskPromptInput): string {
  return [
    ...
    '2. 总结撰写（先模式匹配、后动笔）：mcp__shufa__kb_list 扫「总结模式」组——',
    '   拿转录对照各模式的触发词，命中才叠加（可多选），全不命中只用「兜底·」模式；',
    '   按选中模式的「关联」用 mcp__shufa__kb_get 取知识条目，再用 mcp__shufa__summary_write',
    '   撰写并提交摘要与 labels。summary.json（topic/paragraphs/key_points）引用教师原话、',
    '   术语对照知识条目写准，不要复述规则模板、不要写占位内容；老师没讲过的不要写。',
    '   labels 与摘要一并提交：田字格 label 只标转录明确点到的字（index 见 grid/ink 返回）；',
    ...
    '   告警且可补救（转录点到的格漏标），补一次 summary_write 再重导；返回值里的 result_url',
    '   就是给用户的公开结果链接，结束时把该链接和一句总评转告用户。',
    '4. 用户要求整理/沉淀/修正知识库时，按手册「知识库管理模式」执行：kb_list 看现状 →',
    '   拟变更并复述征得用户同意 → kb_save_*/kb_delete_* 写入（自动记 git 修订）→ 汇报。',
  ].join('\n');
}
