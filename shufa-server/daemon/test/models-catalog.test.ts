...
describe('BUG4 builtin 预设（pi-ai 内嵌 provider 数据）', () => {
  ...
  test('catalog 无缓存：builtin 恒在，fetched_at=null', () => {
    const s = createServices();
    try {
      const catalog = modelCatalog(s.db);
      expect(catalog.fetched_at).toBeNull();
      expect(catalog.presets).toEqual(builtinModelPresets());
      expect(catalog.presets.every((p) => p.source === 'builtin')).toBe(true);
    } finally {
      s.dispose();
    }
  });
...
describe('BUG4 models.dev 刷新与缓存', () => {
  test('映射过滤：只收 http api 的 provider；模型截到上限；缓存与 fetched_at 落 settings', async () => {
    ...
    try {
      ...
      // catalog 合并：builtin 在前 + models.dev 追加；fetched_at 非空。
      const catalog = modelCatalog(s.db);
      ...
      // builtin 前缀不受缓存影响。
      expect(catalog.presets.slice(0, builtinModelPresets().length)).toEqual(builtinModelPresets());
    } finally {
      s.dispose();
    }
  });
  ...
  test('fetchModelsDevPresets 直接映射；网络失败中文报错且缓存不受污染', async () => {
    ...
    try {
      ...
      // builtin 不受影响。
      expect(modelCatalog(s.db).presets.length).toBe(builtinModelPresets().length);
    } finally {
      s.dispose();
    }
  });
...
describe('BUG4 RPC 面：admin.models.catalog / catalogRefresh', () => {
  test('非 admin 403；admin 读取 builtin；刷新经注入 fetch 合并目录', async () => {
    ...
    try {
      ...
      expect(catalog.fetched_at).toBeNull();
      expect(catalog.presets.length).toBe(builtinModelPresets().length);
