/**
 * 自定义视频播放器 + 时间轴实时同步（自 shufa-tool web/src/player.ts 移植，BUG5 结果页复刻）。
 * 交互清单（与打磨版一致）：播放/暂停（按钮 / 点击画面 / 空格，空格仅在播放器可见且
 * 焦点不在交互元素时拦截）、进度条拖拽 seek（已播填充 + 播放位置游标）、旁注活动量
 * 墨迹图层（自 track 顶边向上生长的半透明 SVG，hover 渐显，不与进度条重叠）、
 * 关键帧菱形标记可点击 + hover tooltip、当前/总时长、倍速 Popover 菜单（0.5/1/1.5/2）、
 * 循环开关（默认开）、静音开关；时间轴 seg/kf 按时间混排、活跃条目高亮 +
 * scrollIntoView 跟随（滚动至列表顶部，配合 scroll-snap/scroll-padding 与末尾撑高；
 * 用户手动滚动后 4s 内不打扰）、每条 seg 独立背景进度填充（transform:scaleX）、
 * 点击条目 seek、进度条渲染 kf 菱形。
 * 跨栏跳转：监听 events.ts 的 "shufa:seek"（保持播放状态仅更新进度，元数据
 * 未就绪时挂起至 loadedmetadata 应用）；字幕列表外置到 listHost（结果页布局壳的
 * #dyn-pane）。同步由 rAF（播放中）+ timeupdate/seeked（兜底）驱动；测试钩子：
 * location.hash #seek=X 初始化后跳转并暂停（深链 /r/{id} 走 pathname 路由时仍可用）；
 * window.__testState 暴露同步状态。
 * 移植差异（相对打磨版）：数据类型改用 @zhumo/contracts 的 AnalysisPlayer；
 * assetUrl 由调用方注入（/api/results/{publicId}/assets/* 公开 Range 面）。
 */
import type { AnalysisPlayer, AnalysisPlayerEntry } from "@zhumo/contracts";
...
type SegEntry = Extract<AnalysisPlayerEntry, { type: "seg" }>;
type KfEntry = Extract<AnalysisPlayerEntry, { type: "kf" }>;
...
function entryTime(e: AnalysisPlayerEntry): number {
...
function segHtml(e: SegEntry, idx: number): string {
...
function kfHtml(e: KfEntry, idx: number, assetUrl: (p: string) => string): string {
...
function markHtml(e: KfEntry, idx: number): string {
...
export class ShufaPlayer {
  ...
  <div class="sp-stage">
    ...
    // 字幕列表：布局壳传入 listHost 时外置（#dyn-pane），桌面位于播放器正下方，
    // 移动端作为「动态学习」tab 面板。
  ...
  private setPlayingClass(): void {
    this.root.classList.toggle("sp-playing", !this.video.paused && !this.video.ended);
  }
  ...
  private seekFromItem(target: EventTarget | null): void {
    ...
    if (!el) return;
    const t =
      el.dataset.kind === "seg" ? parseFloat(el.dataset.t0 ?? "0") : parseFloat(el.dataset.t ?? "0");
    this.seekTo(t);
  }
  ...
  /** 测试钩子：URL hash 带 #seek=X 时，元数据就绪后 seek 到 X 并暂停
   *（深链 /r/{id} 走 pathname 路由、hash 空闲时可用；hash 路由下自然 no-op） */
  ...
  /**
   * 测试钩子：URL hash 带 #popover=… 时模拟 hover 并保持 Popover 打开（data-open="1"）。
   ...
   * 三者均不依赖媒体元数据，因此同步执行一次——wheel 计数不可重复派发。
   *（深链 /r/{id} 走 pathname 路由、hash 空闲时可用；hash 路由下自然 no-op）
   */
